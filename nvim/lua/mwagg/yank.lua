-- largely taken from https://gist.github.com/MyyPo/569de2bff5644d2c351d54a0d42ad09f
...
vim.api.nvim_create_autocmd("ModeChanged", {
  group = yank_group,
  pattern = "*",
  callback = function() vim.g.pre_yank_cursor_pos = vim.fn.getcurpos() end,
})
...
-- put value into clipboard on yank
vim.api.nvim_create_autocmd("TextYankPost", {
  group = yank_group,
  pattern = "*",
  callback = function()
    if vim.v.event.operator == "y" then vim.fn.setreg("+", vim.fn.getreg "0", vim.fn.getregtype "0") end
  end,
})
